    ...
    <Box className="flex flex-col mt-3 p-2" >
      ...
      <Box className="group relative w-full">
        ...
        <Box className="absolute hidden top-0 left-0 w-full sm:flex flex-col h-full opacity-0 group-hover:opacity-100 duration-500 group-hover:bg-black/90 items-center p-2 text-gray-300">
            ...
            {/* Title */}
            <Typography className="xl:mt-5 transition ease-in-out delay-150  hover:-translate-y-[1px] hover:scale-109  duration-200 hover:underline cursor-pointer hover:text-blue-600"><Link to={`/movieDetails/${item.id}`}>{item.name}</Link></Typography>
      ...
      <Box className="flex flex-col p-1">
            ...
            {/* Title */}
            <Typography className="text-[14px] mt-2 flex justify-center font-bold transition ease-in-out delay-150  hover:-translate-y-[1px] hover:scale-110  duration-200 hover:underline cursor-pointer hover:text-blue-600"><Link to={`/movieDetails/${item.id}`}>{item.name}</Link></Typography>

            {/* Other */}
            <Box className="flex justify-between items-center mt-2">
                {item.first_air_date? <Typography className="text-[12px]">First Air Date: <span className="font-semibold">{item.first_air_date.slice(0,4)}</span></Typography> : <span className="text-[12px]">First Air Date: N/A</span>}
